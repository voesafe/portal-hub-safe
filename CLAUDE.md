...
- **Deploy:** backend em **produção @4** (`clasp deploy -i AKfycbyeoa-8Vv…`); mudança 100% **aditiva/retrocompatível** (ações antigas intactas, `by` opcional). Rollback: `clasp redeploy AKfycbyeoa-8Vv… -V 1`. Frontend ([escala-cco.html](escala-cco.html)) tem JS inline (sem `?v=` a bumpar) → publica via git push na `main` (GitHub Pages). **Sem otimista** no getLog (só leitura). ⚠️ Cada usuário precisa de **hard-refresh** (Cmd+Shift+R) pós-deploy senão a página em cache não envia o `by` (autor sai vazio no LOG).
