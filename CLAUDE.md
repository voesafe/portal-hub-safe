...
- **Deploy/publicação:** frontend vai pro **GitHub Pages** (git push na main → hub.voesafe.com.br). ⚠️ **O bloqueio de backend acabou:** produção está no **@37** desde 2026-07-27 e a rota `notams` responde (verificado: devolve "Sessão expirada" para token inválido, não mais "Ação desconhecida"). **Sobraram dois passos, os dois fora do código:** configurar `AISWEB_API_KEY`/`AISWEB_API_PASS` nas Propriedades do script e rodar `notamsInstalarTrigger()`. Só depois disso trocar `NOTAMS_ATIVO` para `true` e bumpar o `?v=` do auth.js. Ligar o flag antes da chave deixaria a página no ar sem dado nenhum.
...
- ⚠️ **A fonte local do backend fica em `../horas-voadas-inva-main/backend/Code.gs`, FORA deste repo, e sem git nem clasp.** É uma cópia: quem manda é o editor do Apps Script. Antes de colar qualquer alteração lá, compare com o que está publicado, senão você sobrescreve edição feita direto no editor. Publicar exige nova implantação do web app, o `?v=` do Hub não alcança isso.
