...
- ⚠️ **Exige deploy do backend.** `clasp push` só mexe no @HEAD; produção precisa de `clasp deploy -i AKfycbxpOGXgEJ5…`. Enquanto não subir, o upload responde "Ação desconhecida: salvar-avatar" e o resto do Hub segue normal (a leitura do avatar é tolerante: `idx.AVATAR ? ... : ''`).
